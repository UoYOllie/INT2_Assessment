...
model.add(tf.keras.layers.Conv2D(16, (3, 2), 1, padding='same', activation='relu', input_shape=(256, 256, 3)))
...
model.add(tf.keras.layers.Dense(102))
...
logdir = "logs"
tensorboard_callback = CustomCallback(log_dir=logdir)

hist = model.fit(ds, epochs=1, validation_data=ds_val)
plt.figure()
